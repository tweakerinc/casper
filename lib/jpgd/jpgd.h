...
namespace jpgd
{
	...
	enum
	{
		JPGD_IN_BUF_SIZE = 2048, JPGD_MAX_BLOCKS_PER_MCU = 10, JPGD_MAX_HUFF_TABLES = 8, JPGD_MAX_QUANT_TABLES = 4,
	...
	public:
		enum
		{
			...
			cFlagDisableSIMD = 2,
			// Casper cover path: skip RGBA sample buffers; emit luma 8×8 via copy_luma_block().
			cFlagCoverDecode = 4
		};
		...
		bool is_progressive() const { return m_progressive_flag != 0; }

		void set_yield_callback(void (*fn)()) { m_yield_fn = fn; }

		// After begin_decoding() on a progressive (or sequential) image: inverse-DCT
		// one 8×8 luma block into dst[64] (row-major). Returns 1 on success.
		int copy_luma_block(int block_x, int block_y, uint8* dst8x8);

		int luma_blocks_x() const;
		int luma_blocks_y() const;
	...
	private:
		...
		struct coeff_buf
		{
			...
			int block_size;
			int64_t spill_ofs;  // >=0 when progressive coeffs live on SD/file
		};
		...
		jpgd_block_coeff_t* coeff_buf_getp(coeff_buf* cb, int block_x, int block_y, bool writable = true);
		void load_next_row();
