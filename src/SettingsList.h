...
inline std::vector<SettingInfo> getSettingsList(const SdCardFontRegistry* registry = nullptr) {
  static const std::vector<SettingInfo> baseList = [] {
    ...
    // --- Controls ---
    // Power button first (top of Controls); side/front follow.
    ...
    add(SettingInfo::Enum(StrId::STR_LONG_PRESS_MENU_ACTION, &CrossPointSettings::longPressMenuAction,
            ...
            .withEnumRawValues({CrossPointSettings::LONG_MENU_OFF,
                                ...
                                CrossPointSettings::LONG_MENU_DICTIONARY}));
    ...
    // Side / front controls after Power (submenus order Power → Front → Side).
    ...
    // Auto-upload settings (device UI lives in KOReaderSettingsActivity).
    add(SettingInfo::DynamicEnum(
        StrId::STR_AUTO_UPLOAD_ON_CLOSE, {StrId::STR_NO, StrId::STR_YES},
        [] { return KOREADER_STORE.getAutoUploadOnClose() ? static_cast<uint8_t>(1) : static_cast<uint8_t>(0); },
        [](uint8_t v) {
    ...
    add(SettingInfo::DynamicEnum(
        ...
        [](uint8_t v) {
          if (v == static_cast<uint8_t>(AutoUploadType::PERCENT)) {
            KOREADER_STORE.setAutoUploadType(AutoUploadType::PERCENT);
          } else if (v == static_cast<uint8_t>(AutoUploadType::ADAPTIVE)) {
            KOREADER_STORE.setAutoUploadType(AutoUploadType::ADAPTIVE);
          } else {
            KOREADER_STORE.setAutoUploadType(AutoUploadType::TIME);
          }
          KOREADER_STORE.saveToFile();
        },
